...
export default function PublishedEvent({event}: any) {
    let total = 0;
            ...
            if (event.bettingOptions.length == 1) {    
                return (
                    <Box sx={{ height: 32, width: 1, gap: 0.125, display: 'flex'}}>
                        <Box sx={{ background: 'rgba(39, 174, 96, 0.1)',width: `${event.bettingOptions[0].yesPrice}%`, display: 'flex', justifyContent: 'left' }}>
                            <Typography sx={{padding: '5px', whiteSpace: 'nowrap'}}>{event.bettingOptions[0].yes_no ? event.bettingOptions[0].yes_no.yes : "Yes"} {event.bettingOptions[0].yesPrice}¢</Typography>
                        </Box>
                        <Box sx={{ background: 'rgba(235, 87, 87, 0.1)',width: `${100 - event.bettingOptions[0].yesPrice}%`, display: 'flex', justifyContent: 'right' }}>
                            <Typography sx={{padding: '5px', whiteSpace: 'nowrap'}}>{event.bettingOptions[0].yes_no ? event.bettingOptions[0].yes_no.no : "No"} {100 - event.bettingOptions[0].yesPrice}¢</Typography>
                        </Box>
                    </Box>
                )
            } else {
                            ...
                            return (
                                <Box sx={{ display: 'flex', mt: 0.5, justifyContent: 'space-between' }} key={index}>
                                    ...
                                    <Box sx={{ width: '140px', gap: 0.125, display: 'flex' }}>
                                        <Box sx={{ background: 'rgba(39, 174, 96, 0.1)',width: `${bettingOption.yesPrice}%`, display: 'flex', justifyContent: 'left' }}>
                                            <Typography sx={{padding: '5px', whiteSpace: 'nowrap'}}>{bettingOption.yes_no ? bettingOption.yes_no.yes : "Yes"} {bettingOption.yesPrice}¢</Typography>
                                        </Box>
                                        <Box sx={{ background: 'rgba(235, 87, 87, 0.1)',width: `${100 - bettingOption.yesPrice}%`, display: 'flex', justifyContent: 'right' }}>
                                            <Typography sx={{padding: '5px', whiteSpace: 'nowrap'}}>{bettingOption.yes_no ? bettingOption.yes_no.no : "No"} {100 - bettingOption.yesPrice}¢</Typography>
                                        </Box>
                                    </Box>
                                </Box>
                            ) 
